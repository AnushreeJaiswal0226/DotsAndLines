//html elements with some UI elements as well
...
// colours
const COLOR_BOARD = "darkslategray";
const COLOR_BORDER = "darkseagreen";
const COLOR_COMP = "azure";
const COLOR_COMP_LIT = "darkgoldenrod";
const COLOR_DOT = "palegreen";
const COLOR_PLAY = "lightyellow";
const COLOR_PLAY_LIT = "darkcyan";
const COLOR_TIE = "black";

// text
const TEXT_COMP = "Computer's Turn";
const TEXT_COMP_SML = "Comp";
const TEXT_PLAY = "Player's Turn";
...
function drawScores() {
    ...
    if (timeEnd > 0) {
        ...
        if(scoreComp < scorePlay) {
            gameovertxt.innerHTML = "<.<. Player Wins >.>.";
        }
        else{
            gameovertxt.innerHTML = ">. Computer Wins .<";
        }
...
function drawText(text, x, y, color, size) {
    ctx.fillStyle = color;
    ctx.font = size + "px Offside, cursive";
    ctx.fillText(text, x, y);
}

function getColor(player, light) {
    if (player) {
        return light ? COLOR_PLAY : COLOR_PLAY_LIT;
    } else {
        return light ? COLOR_COMP : COLOR_COMP_LIT;
    }
}
...
//drawing board before game starts
drawBoard();

//staring game from the start
